...
description: Builds an "Ultimate Single Pager" — one self-contained HTML file (HTML + internal CSS + internal JS, no external CSS/JS except the allowed CDN trio: Font Awesome icons, Google Fonts, and the Google Map embed) with 10 business sections (Home, About, Services, Why Choose Us, FAQ, Testimonials, Gallery, Google Map, Contact, Footer), floating WhatsApp button, a contact form that sends enquiries to WhatsApp via JavaScript, full local-SEO (title/meta/OG/Twitter/JSON-LD, "Best {Service} in {City}" keywords, H1-H3 hierarchy, lazy loading), and AI-generated images through the local Google Flow engine wired in as HOSTED LINKS. You give ONLY the client's business information; the skill writes the entire site structure, copy, and SEO blueprint itself. Use when the user asks for a single-page site, landing page, one-file website, business website, one-pager, pure HTML page, or any site that must be a single .html file with no backend/build step.
---

# Ultimate Single-Pager Studio (AI Images + Local SEO + Client-Ready Features)

## Mission

One short prompt (just the client's business info) → **ONE `.html` file** that is a genuine piece of art: premium, unique, authentic, mobile-first, with the full local-SEO stack baked in — and every client-facing feature the business actually needs: floating WhatsApp button, a contact form that lands in WhatsApp (no backend), an embedded Google Map, 6+ service cards each with its own image and icon, smooth scrolling, sticky navbar, hover effects and animations. Images are generated in the background **in parallel** with the build, then wired in as **HOSTED LINKS** (anonymous free host by default; client hosting when provided).
...
2. **Phase 2 — Content + SEO engine:** the skill writes the full section outline, every word of copy (1,500+ words), the primary keyword, and the complete head/JSON-LD SEO blueprint.
3. **Phase 3 — Build:** start the image pipeline FIRST (background, parallel), then hand-code the HTML/CSS/JS to the client-standard spec below, auto-integrate the hosted image URLs, run the mandatory SEO + feature self-check, and deliver.
...
5. **Products / services to feature** — names, one line each (aim for at least 6 so the Services grid is full; fewer is fine, top up with genuine adjacent services if the client agrees)
6. **Contact** — phone, email, address, **WhatsApp number** (the exact digits, with country code — this powers the floating button AND the form), Google Maps location (address or business name for the embed), socials (whatever exists)
...
If the user says "you decide" or leaves a field blank, fill it with judgment and note it in the summary. Never invent contact details that will embarrass them — use real ones; if unknown, use tasteful placeholders they can swap. If no WhatsApp number is given, still build the floating button wired to the phone number's `wa.me` link and flag it in the summary.
...
Derive ONE primary keyword: `<core service> in <City>` — e.g. "home tuition in Jaipur", "café in Chakala", "gold jewellery in Alwar", "bike service in Dehradun". Pick the single most valuable search phrase for this business. Everything below hangs off it. The page must also naturally carry the three local-SEO variants somewhere in the copy: **"Best {Service} in {Location}"**, **"Best {Service} in {City}"**, **"Best {Service} in {State}"** — at least two of the three as literal phrases, the third as a natural variant.

### 2.2 The 10-section outline — write real copy for each

1. **Home (hero)** — **full-width hero banner** (edge-to-edge image), brand name (loudest text), one-line promise, primary CTA (call / WhatsApp / visit)
2. **About Us** — the story, the people, the place; specific and human, not generic. Must include an attractive image card / image box
3. **Services** — **at least 6 service cards**, each with its **own image** AND a **Font Awesome icon**, a short name and a real one-line description
...
7. **Gallery** — 6–8 images (AI-generated or client photos), laid out as an image grid
8. **Google Map** — a **responsive Google Map embed** (the allowed iframe exception) showing the business location; if the client is fully offline or the embed is impossible, fall back to a hand-drawn inline SVG map and say so in the summary
9. **Contact Us** — full contact block (phone, WhatsApp, email, address, hours) + the **WhatsApp enquiry form** (Name, Phone, Email, Service, Message — submit composes a `wa.me` message via JavaScript, no backend/PHP)
10. **Footer** — social media icons, quick links, contact details, copyright line, and the exact MBG Card credit (see §3.2)
...
- **Keyword discipline:** repeat the exact primary keyword phrase throughout; put it in **≥ 2 H2/H3 headings**; aim for **≥ 1.5% keyword density**; name the **city repeatedly** in the copy; work in the "Best … in {Location/City/State}" variants naturally.
...
| **Open Graph** | ≥ 6 tags: `og:title`, `og:description`, `og:type`, `og:url`, `og:locale`, `og:site_name` (+ `og:image` as the hero hosted URL) |
| **Twitter card** | `twitter:card` = `summary_large_image`, `twitter:title`, `twitter:description`, `twitter:image` |
| **H1** | EXACTLY one, mirroring the title: `Best <keyword> in <City>` (not a slogan) |
| **H1–H3 hierarchy** | One `<h1>`; every major section gets an `<h2>`; sub-blocks use `<h3>`; nothing skips a level |
...
Before building, assign each visual slot an aspect from the ratio table (§3.3) and a one-line art direction (subject + setting + palette, filled into the prompt template). Typical slot list for the full spec: `hero` (16:9), `about` (4:3), `service-1 … service-6` (square), `gallery-1 … gallery-6` (square/mixed). This becomes the `site-images.json` spec the background pipeline runs (§3.3).
...
Follow the `frontend-design` skill's principles: one signature element the page is remembered by, one accent color, max two typefaces, editorial spacing, strong type hierarchy, real imagery, restrained motion (a load sequence, scroll reveals, hover micro-interactions), sticky navbar, smooth scrolling, and a professional colour scheme. The business name is the loudest text in the hero.
...
**Anti-slop rules (hard):** no AI-purple gradients or generic SaaS card grids; no beige-cream + serif + terracotta default unless the brand genuinely is that; no near-black + acid accent default; no glossy glassmorphism everywhere; no em-dashes in copy; no "Trusted by / 100+ happy customers" fake-proof strips unless real; no fake-precision numbers; no generic names; no AI-gradient "slop" backgrounds. If a choice reads like what any AI would do for any business, change it.

### 3.2 Hard constraints (non-negotiable)

- Exactly ONE `.html` file: HTML + internal CSS + internal JavaScript. **No external CSS/JS files, no build step, no backend/PHP.**
- **Allowed CDN exceptions (the only external resources permitted):** Font Awesome icons CDN, Google Fonts CDN, and the Google Maps embed iframe. Everything else must be inline. No other CDNs, no analytics, no tracking, no third-party scripts.
- **No external links:** contact links (`tel:`, `mailto:`, `https://wa.me/`) and the Google Map embed are the only allowed anchors/iframes. JSON-LD `url`/`sameAs` metadata is fine — it's metadata, not an embed.
...
- Responsive (mobile-first, desktop/tablet/mobile), semantic HTML, valid CSS, keyboard-accessible, visible focus, `prefers-reduced-motion` respected.
- **Floating WhatsApp button** — fixed position, visible across the ENTIRE website at every scroll position, links to `https://wa.me/<digits>` with a pre-filled message, styled as a recognizable WhatsApp circle, with an accessible label.
- **Contact form** — Name, Phone, Email, Service, Message; on submit it composes the enquiry into a WhatsApp message and opens `wa.me/<digits>?text=<encoded>` via JavaScript. No backend. Validate fields with native HTML5 + a little JS; show a clear inline message when the number is missing.
- **Footer — exact line** (compute the year with one inline JS line, e.g. `<span id="yr"></span>` set to `new Date().getFullYear()`):
  ...
  Plus: social media icons (Font Awesome, `aria-label`ed), quick links (anchor to in-page sections), and contact details.
...
```json
{
  "slots": [
    ...
    { "key": "about", "prompt": "<template-filled>", "size": "1536x1024" },
    { "key": "service-1", "prompt": "<template-filled>", "size": "1024x1024" }
  ]
}
```
...
| Square (gallery, service, about) | `1024x1024` | 1:1 |
...
### 3.4 Mandatory self-check before delivery (the SEO + feature audit)
...
**Headings & Structure**
- [ ] Exactly one `<h1>` that mirrors the title (`Best <keyword> in <City>`)
- [ ] H1–H3 hierarchy: every major section has an `<h2>`, sub-blocks use `<h3>`, nothing skips a level
...
- [ ] Local-SEO variants present: "Best {Service} in {Location}" / "Best {Service} in {City}" / "Best {Service} in {State}"

**Client features**
- [ ] Floating WhatsApp button present and visible across the whole page (`wa.me` link, aria-label)
- [ ] Contact form (Name, Phone, Email, Service, Message) submits to WhatsApp via JS — no backend
- [ ] Responsive Google Map embed (or stated SVG fallback in the summary)
- [ ] Services section has ≥ 6 cards, each with an image AND a Font Awesome icon
- [ ] Every major section has an attractive image card / image box
- [ ] Sticky navbar, smooth scrolling, hover effects, entrance animations, professional colour scheme
- [ ] 100% responsive (desktop / tablet / mobile — no horizontal scroll, readable text, tap-friendly targets)

**Zero-dependency & footer**
- [ ] No external CSS/JS except the allowed CDN trio (Font Awesome, Google Fonts, Google Map embed); no other third-party scripts
- [ ] No external links except `tel:` / `mailto:` / `wa.me` (and the map embed)
