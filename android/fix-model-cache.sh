    ...
    try {
      ...
      if (at >= 0) {
        ...
        if (atOld >= 0) {
          ...
          ok('claude picker patch applied (every route now in /model)');
        } else ok('claude picker already patched');
